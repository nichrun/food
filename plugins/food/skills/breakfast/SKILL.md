---
name: breakfast
description: Choose one definitive breakfast from the user's available food, preferences, approximate meal plan, leftovers, and recipe memory, then give a concise ready-to-cook recipe. Use when the user asks what to have for breakfast or invokes the breakfast meal shortcut.
---
...
- Obey allergies and hard restrictions. Respect the explicit request for breakfast even when the profile says the user does not usually eat it.
...
Start with `For breakfast, we're making **<meal>**.` Sound confident, kind, and appetising.
