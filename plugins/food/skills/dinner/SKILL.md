---
name: dinner
description: Choose one definitive dinner from the user's available food, preferences, approximate meal plan, leftovers, and recipe memory, then give a concise ready-to-cook recipe. Use when the user asks what to have for dinner or invokes the dinner meal shortcut.
---
...
- Obey allergies and hard restrictions. Respect the explicit request for dinner even when the profile says the user does not usually eat it.
...
Start with `For dinner, we're making **<meal>**.` Sound confident, kind, and appetising.
